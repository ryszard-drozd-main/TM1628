...
: buffer {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}
, seg_addr {0x00,0x01,0x07,0x02,0x03,0x04,0x05}//no bit of digital segments
//          SG0  SG1  SG2  SG3  SG4  SG5  SG6  SG7  DVD  VCD  MP3  PLY  PAU  PBC  RET  DTS  DDD  CL1  CL2   //name   -|
, led_addr {0x03,0x0B,0x0D,0x07,0x05,0x09,0x0D,0x01,0x01,0x03,0x05,0x09,0x0b,0x07,0x00,0x08,0x06,0x02,0x04}//adress -| for the signs's and disc's leds
, led_val  {0x01,0x01,0x01,0x01,0x01,0x01,0x02,0x01,0x02,0x02,0x02,0x02,0x02,0x02,0x40,0x40,0x40,0x40,0x40}//byte   -|
{
...
size_t TM1628::write(byte chr){
	//if(_curpos<0x07) {
  if(TM1628::_curpos<0x07) {
		//setChar(_curpos, chr);
    setChar(TM1628::_curpos, chr);
		//_curpos++;
    TM1628::_curpos++;
	}
}
